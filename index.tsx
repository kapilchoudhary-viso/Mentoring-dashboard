  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 shadow-sm flex flex-col h-full overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center bg-gray-50/50">
        <div>
          <h2 className="text-lg font-bold text-gray-900">Student without Mentor</h2>
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-lg border shadow-sm overflow-hidden"><table className="min-w-full divide-y divide-gray-200"><thead className="bg-gray-50"><tr>
          {isBulkMode && <th className="px-6 py-3 w-10"><input type="checkbox" disabled={unallocatedBatchStudents.length === 0} onChange={(e) => e.target.checked ? unallocatedBatchStudents.forEach((s: any) => onToggleSelect(s.id)) : unallocatedBatchStudents.forEach((s: any) => onToggleSelect(s.id))} /></th>}
          <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Reg ID / Name</th>
          ...
          <tbody className="bg-white divide-y divide-gray-200">{batchStudents.map((s: any) => (<tr key={s.id}>
            {isBulkMode && <td className="px-6 py-4">{!s.assignedMentor && <input type="checkbox" checked={selectedIds.has(s.id)} onChange={() => onToggleSelect(s.id)} />}</td>}
...
const MentorDirectoryView: React.FC<{ mentors: Mentor[], students: Student[], initialSearchTerm?: string, initialSelectedMentorId?: string }> = ({ mentors, students, initialSearchTerm = '', initialSelectedMentorId = '' }) => {
  ...
  const saveEdit = () => {
    if (selectedMentor) {
      // Update mentor data (in a real app, this would call an API)
...
const MentorAssignmentModal = ({ isOpen, onClose, students, mentors, onAssign }: any) => {
  ...
  // Reset filters and state when modal opens/closes
  useEffect(() => {
    if (isOpen) {
      setSearchTerm('');
  ...
  // Determine if this is a reassignment (any selected student already has a mentor)
  const isReassignment = (students || []).some((s: any) => s.assignedMentor);
  const title = students?.length > 1
    ? `Assign Mentor to ${students.length} Students`
    : `Assign Mentor to ${students?.[0]?.name}`;
        ...
        <div className="p-4 bg-gray-50 border-b space-y-3">
          ...
          {isReassignment && (
            <div className="bg-yellow-50 border border-yellow-200 rounded p-3 mb-2">
              <label className="block text-sm font-bold text-yellow-800 mb-1">Reason for Reassignment <span className="text-red-500">*</span></label>
              <input
                className="w-full border border-yellow-300 rounded p-2 text-sm focus:ring-2 focus:ring-yellow-400 outline-none"
                placeholder="Why are these students being reassigned?"
        ...
        <div className="flex-1 overflow-y-auto p-4">
          <div className="space-y-3">
            {filteredMentors.map((m: any) => (
              <div key={m.id} className="border p-4 rounded-lg flex justify-between items-center hover:bg-gray-50">
                <div>
                  <div className="font-bold">{m.name}</div>
                  <div className="text-sm text-gray-500">{m.department} • {m.currentLoad}/{m.capacity}</div>
                </div>
                <button
                  onClick={() => handleAssign(m.id)}
                  disabled={m.status === 'Full'}
                  className={`px-4 py-2 rounded text-sm font-medium ${m.status === 'Full'
            ...
            {filteredMentors.length === 0 && (
              <div className="text-center py-8 text-gray-500">No mentors match filters.</div>
            )}
          </div>
        </div>
      </div>
